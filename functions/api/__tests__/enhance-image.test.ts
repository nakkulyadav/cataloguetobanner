...
import { onRequestPost } from '../enhance-image'
...
function makeRequest(body: unknown): Request {
  return new Request('https://worker.example/api/enhance-image', {
    method: 'POST',
...
describe('onRequestPost — enhance-image Worker', () => {
  describe('validation', () => {
    ...
    it('returns 400 when imageData is not valid base64', async () => {
      const resp = await onRequestPost({
        request: makeRequest({ imageData: '!!!not-base64!!!' }),
        env: makeEnv(),
      })
      expect(resp.status).toBe(400)
      const body = await resp.json() as { error: string }
  ...
  describe('R2 cache miss', () => {
    it('calls HF, stores result in R2, returns { url, cached: false } for imageUrl', async () => {
      vi.spyOn(globalThis, 'fetch')
        ...
        .mockResolvedValueOnce(pngFetchResponse())  // HF enhance call
    ...
    it('uses esrgan: prefix for imageUrl cache key and esrgan-data: for imageData', async () => {
      vi.spyOn(globalThis, 'fetch')
        .mockResolvedValueOnce(pngFetchResponse()) // upstream fetch
        .mockResolvedValueOnce(pngFetchResponse()) // HF call

      await onRequestPost({ request: makeRequest({ imageUrl: IMAGE_URL }), env: makeEnv() })
      ...
      mockR2.getFromR2.mockResolvedValue(null)
      mockR2.storeInR2.mockResolvedValue(undefined)
      vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(pngFetchResponse())

      await onRequestPost({ request: makeRequest({ imageData: btoa('bytes') }), env: makeEnv() })
      const dataInput = mockR2.hashKey.mock.calls[0]?.[0] as string

      expect(urlInput.startsWith('esrgan:')).toBe(true)
      expect(dataInput.startsWith('esrgan-data:')).toBe(true)
    })
  })
  ...
  describe('HF error handling', () => {
    it('returns 502 when HF responds with non-OK status', async () => {
      ...
      const body = await resp.json() as { error: string }
      expect(body.error).toBe('HuggingFace inference error')
    })
    ...
    it('returns 502 with loading message for 503 with estimated_time', async () => {
      vi.spyOn(globalThis, 'fetch')
        ...
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ error: 'loading', estimated_time: 45 }), {
            status: 503,
            headers: { 'content-type': 'application/json' },
          }),
        )
      ...
      expect(body.detail).toMatch(/45s/i)
    })
    ...
    it('returns 502 when upstream image fetch fails', async () => {
      ...
      expect(body.error).toMatch(/fetch failed|upstream/i)
    })
  })
})
